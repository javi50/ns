    ...
    try{
      ...
      if (!tracks.length)
        return message.channel.send(new MessageEmbed()
          ...
          .setColor("BLUE").addField(`**0) CURRENT TRACK**`, `**${player.queue.current.title.substr(0, 60)}** - \`${player.queue.current.isStream ? `LIVE STREAM` : format(player.queue.current.duration).split(` | `)[0]}\`\n*request by: ${player.queue.current.requester.tag}*`)
      ...
      if (tracks.length < 15)
        return message.channel.send(new MessageEmbed()
          ...
          .setColor("BLUE").setDescription(tracks.map((track, i) => `**${++i})** **${track.title.substr(0, 60)}** - \`${track.isStream ? `LIVE STREAM` : format(track.duration).split(` | `)[0]}\`\n*requested by: ${track.requester.tag}*`).join(`\n`))
        ).then(msg => {
          try {
            msg.delete({
              timeout: 5000
