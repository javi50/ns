  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      fs.writeFile(`./botconfig/songoftheday.json`, JSON.stringify(ss, null, 3), (e) => {
        if (e) {
          console.log(String(e.stack).red);
          return message.channel.send(new MessageEmbed().setFooter(client.user.username + " | by: milrato.eu", client.user.displayAvatarURL()).setColor("RED").setTitle(`${emoji.msg.ERROR}  ERROR Writing the File`).setDescription(`\`\`\`${e.message}\`\`\``))
        }
        ...
        return message.channel.send(new MessageEmbed().setFooter(client.user.username + " | by: milrato.eu", client.user.displayAvatarURL()).setColor("BLUE").setTitle(`${emoji.msg.SUCCESS}  Successfully set the new Track of the Day`))
      });
