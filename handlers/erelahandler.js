    ...
    client.manager
      ...
      .on("playerCreate", async (player) => {
          ...
          var embed = new MessageEmbed();
            ...
            if(irc) return;
          client.channels.cache.get(player.textChannel).send(embed.setColor("BLUE")).catch(e=>console.log("this prevents a crash"));
      })
      .on("playerMove", async (player, oldChannel, newChannel) => {
        if (!newChannel) {
            var embed = new MessageEmbed().setColor("RED").setFooter(client.user.username + " | by: milrato.eu", client.user.displayAvatarURL());
            ...
            client.channels.cache.get(player.textChannel).send(embed).then(msg => {
              ...
              collector.on(`collect`, async (reaction, user) => {
                try {
                  ...
                  if (!channel) return message.channel.send(new MessageEmbed().setColor("RED").setFooter(client.user.username + " | by: milrato.eu", client.user.displayAvatarURL()).setTitle(`${emoji.msg.ERROR} ERROR | You need to join a voice channel.`));
                  ...
                  if (player && channel.id !== player.voiceChannel) return message.channel.send(new MessageEmbed().setColor("RED").setFooter(client.user.username + " | by: milrato.eu", client.user.displayAvatarURL()).setTitle(`${emoji.msg.ERROR} ERROR | I am already playing somewhere else!`).setDescription(`You can listen to me in: ${message.guild.channels.cache.get(player.VoiceChannel).name}`));
